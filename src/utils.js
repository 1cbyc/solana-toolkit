...
class UtilsManager {
  ...
  lamportsToSol(lamports) {
    try {
      ...
      throw new SolanaToolkitError(
        `Failed to convert lamports to SOL: ${error.message}`,
        'LAMPORTS_CONVERSION_ERROR',
        { lamports, error: error.message }
      );
    }
  }
  ...
  solToLamports(sol) {
    try {
      ...
      return {
        sol: num,
        lamports: lamports,
        formatted: `${lamports} lamports`
      };
    } catch (error) {
      throw new SolanaToolkitError(
        `Failed to convert SOL to lamports: ${error.message}`,
        'SOL_CONVERSION_ERROR',
        { sol, error: error.message }
      );
    }
  }
  ...
  generateRandomKeypair() {
    try {
      ...
      this.logger.info('Random keypair generated', { 
      ...
      throw new SolanaToolkitError(
        `Failed to generate random keypair: ${error.message}`,
  ...
  encodeBase58(data) {
    try {
      ...
      if (typeof data === 'string') {
        ...
        throw new Error('Data must be a string, array, Uint8Array, or Buffer');
      }
      ...
      throw new SolanaToolkitError(
        `Failed to encode Base58: ${error.message}`,
        'BASE58_ENCODE_ERROR',
        { error: error.message }
      );
    }
  ...
  decodeBase58(encodedData) {
    try {
      ...
      const decodedString = Buffer.from(decoded).toString('utf8');
      ...
      throw new SolanaToolkitError(
        `Failed to decode Base58: ${error.message}`,
        'BASE58_DECODE_ERROR',
        { encodedData, error: error.message }
      );
    }
  }
  ...
  encodeBase64(data) {
    try {
      ...
      throw new SolanaToolkitError(
        `Failed to encode Base64: ${error.message}`,
        'BASE64_ENCODE_ERROR',
        { error: error.message }
      );
    }
  ...
  generateMnemonic(length = 12) {
    try {
      if (![12, 15, 18, 21, 24].includes(length)) {
        throw new Error('Mnemonic length must be 12, 15, 18, 21, or 24 words');
      }
      ...
      const words = [
        ...
        'voice', 'void', 'volcano', 'volume', 'vote', 'voucher', 'vow', 'voyal', 'vulnerable', 'wad',
        'wag', 'wage', 'wait', 'walk', 'wall', 'walnut', 'want', 'warfare', 'warm', 'warrior',
        ...
        'will', 'willing', 'win', 'window', 'wine', 'wing', 'wink', 'winner', 'winter', 'wire',
      ...
      throw new SolanaToolkitError(
        `Failed to generate mnemonic: ${error.message}`,
  ...
  formatNumber(number, decimals = 2) {
    try {
      ...
      throw new SolanaToolkitError(
        `Failed to format number: ${error.message}`,
        'NUMBER_FORMAT_ERROR',
        { number, decimals, error: error.message }
      );
    }
  }
  ...
  formatCurrency(amount, currency = 'USD', locale = 'en-US') {
    try {
      ...
      throw new SolanaToolkitError(
        `Failed to format currency: ${error.message}`,
        'CURRENCY_FORMAT_ERROR',
        { amount, currency, locale, error: error.message }
      );
    }
  }
  ...
  generateRandomBytes(length = 32) {
    try {
      ...
      throw new SolanaToolkitError(
        `Failed to generate random bytes: ${error.message}`,
        'RANDOM_BYTES_ERROR',
        { length, error: error.message }
      );
    }
